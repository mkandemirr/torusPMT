...
// Destructor
GLG4TorusStack::~GLG4TorusStack() { SetAllParameters(0, NULL, NULL, NULL); }
...
G4int GLG4TorusStack::FindInOrderedList(double z_lu, const double *z, int n) {
  ...
  if (n <= 0 || z == NULL) {
    G4Exception(__FILE__, "Invalid Parameter", FatalException,
                "Error in GLG4TorusStack::FindInOrderedList - "
...
G4double GLG4TorusStack::DistanceToIn(const G4ThreeVector &p, const G4ThreeVector &v) const {
  ...
  if (p.z() > z_edge[n] - surfaceTolerance) {  // above top of torus stack
    ...
    if (v.z() <= 0.0) {
      DEBUG_GLG4TorusStack(return 1.04e100;) return kInfinity;
    }
    iedge = 0;
    idir = +1;
  } else {
  ...
  if (rmin2 > square(max_rho + myRadTolerance)) {
    DEBUG_GLG4TorusStack(return 1.05e100;) return kInfinity;
  }
...
G4ThreeVectorList *GLG4TorusStack::CreateRotatedVertices(const G4AffineTransform &pTransform,
  ...
  if (vertices) {
    ...
    G4Exception(__FILE__, "Out of Memory", FatalException,
                "GLG4TorusStack::CreateRotatedVertices Out of memory - Cannot "
                "alloc vertices");
  }
  return vertices;
}
